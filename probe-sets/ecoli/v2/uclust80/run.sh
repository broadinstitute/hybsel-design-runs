    ...
    for m in 2; do
        for e in 25; do
            ...
            for x in input/*; do
                ...
                cmd="make_probes.py -pl 75 -ps 25 -l 75 -m $m -e $e --island_of_exact_match 30 --expand_n --small_seq_min 60 --skip_reverse_complements --skip_adapters -d custom:${x} -o probes/m${m}-e${e}-iem30-expandn/${f}.fasta --max_num_processes 2 --verbose > log/m${m}-e${e}-iem30-expandn.${f}.out 2> log/m${m}-e${e}-iem30-expandn.${f}.err"
                echo "$cmd" >> $COMMANDS
            done
        done
    done
